...
struct alignas(16U) lightData
{
    ...
    int angularPos = 0; //this is more of an offset
...
public:
  ...
  ~Light();
    ...
    void updatePosition(vec3 Pos, mat4 MVP); //sets position on the orbit
    void update(Shader* shad);
    void draw(Shader* shad);
    //helper for loading sphere
    void loadMesh(Mesh* m, Shader* shad);
...
private: //data we don't need to give out
    ...
    glm::mat4 model = glm::mat4(1); //for displaying the light
