...
void LightManager::updateLights(orbit* orbit_, int iterator)
{
    //this just updates the drawing and position of the lights
    for (int i = 0; i < 16; ++i) //16 is the max num lights
    {
        //this gives the new position of the light, so need to set it here
        orbitSegment lightLoc = orbit_->getSegment(iterator + lights[i]->data.angularPosition);
        lights[i]->updatePosition(lightLoc);
        lights[i]->update();
        setPosition(i, glm::vec4(lightLoc.getStart(), 1.0f));
        setDirection(i, glm::vec4(lightLoc.getStart(), 1.0f));
    }
...
void LightManager::setAngularPosition(int index, int val)
{
    dataGlobal.l[index].angularPosition = val;
    lights[index]->data.angularPosition = val;
}

void LightManager::setInnerAngle(int index, float val)
{
    dataGlobal.l[index].innerAngle = val;
    lights[index]->data.innerAngle = val;
}

void LightManager::setOuterAngle(int index, float val)
{
    dataGlobal.l[index].outerAngle = val;
    lights[index]->data.outerAngle = val;
}
